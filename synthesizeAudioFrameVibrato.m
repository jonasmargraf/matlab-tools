...
% (with additional vibrato).
function [audioFrame] = synthesizeAudioFrameVibrato(   frequenciesStart, ... 
                                                            frequenciesEnd, ...
                                                            amplitudesStart, ...
                                                            amplitudesEnd, ...
                                                            frameSize, ...
...
for i = 1:nPartials
    ...
    % Calculate sine signal for this partial with added vibrato
    audioFrame(i, :) = amplitude .* (sin(((2 * pi) * f0*i / fs) * ...
                                        (t + (frequency-f0*i) + vibrato)));
